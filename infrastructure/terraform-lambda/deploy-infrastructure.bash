...
echo Deploying Lambda infrastructure via Terraform...

cd infrastructure/terraform-lambda
TF_VAR_FILE="${TF_VAR_FILE:-environments/prod-lambda/terraform.tfvars}"

terraform init \
  -reconfigure \
  -backend-config "bucket=${REMOTE_STATE_BUCKET}" \
  -backend-config "key=${TF_STATE_KEY:-reference-architecture-lambda}" \
