...
Helmet sets a strict same-origin CSP by default. All deployments run behind the Cloudflare proxy, which auto-injects the Cloudflare Web Analytics beacon, so `script-src` always allows `static.cloudflareinsights.com` and `connect-src` always allows `cloudflareinsights.com`. Because the Auth0 SPA SDK exchanges the authorization code for tokens via `fetch` to the Auth0 origin and renews tokens through a hidden iframe, the backend additionally adds the Auth0 issuer origin to `connect-src` and `frame-src` **only** when `AUTH_PROVIDER=oidc` (derived from `OIDC_ISSUER`). This is handled in code — no per-deployment CSP config is needed — but it means an OIDC deployment must have a correct `OIDC_ISSUER`.
